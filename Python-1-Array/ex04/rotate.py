...
def rotate(img: np.ndarray, angle: int = 90) -> np.ndarray:
    """
    ...
    """
    if not isinstance(img, np.ndarray):
        raise TypeError("Image muste be a np.ndarray")
    if angle == 90:
        rotated = np.transpose(img, (1, 0, 2))[::-1]
    elif angle == 180:
        rotated = img[::-1, ::-1]
    elif angle == 270:
        rotated = np.transpose(img, (1, 0, 2))[:, ::-1]
...
def main():
    """
    Load, zoom, convert to grayscale, rotate and display image.
    """
